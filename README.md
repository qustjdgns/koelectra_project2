# SOOP 유저 게시글 분석


# 1. 서론 (Introduction)

## 문제 정의 및 프로젝트 개요

SOOP 소통센터에는 매일 많은 유저 게시글이 지속적으로 등록된다.
운영팀과 고객센터는 게시글의 내용을 확인하며 유형을 구분하고, 필요한 경우 직접 대응한다.
그러나 게시글 수가 늘어날수록 모든 내용을 빠르게 검토하는 데 한계가 발생한다.
이 과정에서 일부 이슈는 대응이 늦어지거나, 우선순위 판단이 어려워지는 문제가 생긴다.
특히 유사한 불만이나 오류가 반복됨에도 이를 체계적으로 집계하기 어렵다.
그 결과, 운영은 정량적 분석보다는 담당자의 경험에 의존하는 경향이 나타난다.

### 주요 목표

본 프로젝트의 주요 목표는 SOOP 유저 게시글을 자동으로 분류하고 분석하여,
운영팀과 고객센터가 게시글을 보다 효율적으로 관리할 수 있도록 지원하는 것이다.

이를 위해 게시글 텍스트를 기반으로
**기능건의, 기술오류, 정책비판, 공지, 기타** 의 5개 유형으로 분류하는
KoELECTRA 기반 딥러닝 분류 모델을 구축한다.

또한 단순 분류에 그치지 않고,
라벨별 게시글을 대상으로 토픽 모델링을 수행하여
유저들이 실제로 어떤 문제를 반복적으로 제기하는지 구조적으로 분석한다.

궁극적으로 본 프로젝트는
게시글 유형 분포와 주요 이슈 흐름을 데이터로 시각화하여,
운영 의사결정을 보다 정량적으로 지원하는 것을 목표로 한다.

또한 단순 분류를 넘어 토픽 모델링 통해  
유저가 체감하는 서비스 문제 구조를 데이터로 해석하는 것을 목표로 한다.

---

### 1.2 분석 대상 데이터

- 데이터 소스: SOOP 소통센터 (유저 게시판)
- 수집 목표량: 약 25,000 ~ 30,000건
- 실제 전처리 후 데이터 수: **24,905건**


---

# 2. 데이터 수집 및 크롤링

## 2.1. 데이터 수집 방법 및 코드

대규모 게시글 목록을 페이지네이션 방식으로 크롤링하기 위해 Python의 Selenium 라이브러리를 사용한다.

- **크롤링 도구** : Selenium + Chrome Driver
- **수집 대상 항목** : `post_id`, `title`, `author`, `date`, `views`, `recommends`, `detail_url`, `content` (상세 페이지에서 추출)

### 크롤링 코드 

크롤링 코드의 핵심은 목록에서 링크를 획득하고 상세 페이지의 본문을 추출하는 2단계 크롤링이다.

```python
# CONTENT_SELECTOR가 정확해야 한다. (현재 추정값: div.view_contents)
CONTENT_SELECTOR = "div.view_contents" 

def crawl_detail_content(driver, item):
    # 상세 페이지로 이동하여 본문 텍스트를 추출한다.
    try:
        driver.get(item['detail_url'])
        time.sleep(SLEEP_TIME_DETAIL) 

        # 1. 본문 텍스트 추출 (가장 중요한 CSS 선택자)
        content_element = driver.find_element(By.CSS_SELECTOR, CONTENT_SELECTOR) 
        item['content'] = content_element.text.strip()

    except NoSuchElementException:
        item['content'] = "본문 영역 찾기 실패" 
    # ... (데이터 저장) ...
```


- 크롤링 코드: `soop.py` 
- 수집 데이터: 저작권 문제로 업로드하지 않는다.
- 수집 기간: **2018-09-16 ~ 2025-11-26**
- 실제 전처리 후 데이터 수: **24,905건**
  
---

### 2.2 수집 결과 요약

| 항목 | 목표 | 실제 |
|------|------|------|
| 게시글 수 | 25,000건 이상 | **24,905건** |

---

## 3. 데이터 라벨링 과정

### 3.1 샘플링 전략
전체 데이터 중 2,000건 이상을 수동 라벨링 대상으로 선정하였다. 무작위 샘플링과 전략적 샘플링을 병행하여 클래스 불균형을 완화하고, 중요 이슈 데이터를 충분히 확보하였다.

전략 기준: 조회 수 상위 10%, 추천 수가 많은 게시글과 더불어, 운영 리스크가 높은 **'기술오류'** 와 '정책비판' 클래스의 게시글을 우선 추출하기 위해 아래의 핵심 키워드 목록을 활용하였다.

### 3.2 라벨 정의 및 초기 분류
수동 라벨링 작업의 효율성을 높이기 위해, 게시글 content 텍스트에 기반한 키워드 매칭 방식으로 초기 분류 작업을 수행하였다. 특히 운영자 공지(3)나 기술 오류(1) 등 중요 클래스를 우선적으로 필터링하는 데 활용되었다.

활용된 이슈 유형 분류 키워드 목록

| 라벨 | 클래스 | 주요 키워드 | 분류 기준 |
|------|--------|--------------|------------|
| 0 | 기능건의 | 개선, 요청, UI, 혜택, 포인트 | 기능 추가, 사용자 편의 개선 요청 |
| 1 | 기술오류 | 오류, 버그, 결제, 환불, 팅기는 | 서비스 장애, 결제 문제, 기술 결함 |
| 2 | 정책비판 | 블랙, 차단, 제재, 정책, 숨기기 | 운영 정책 불만, 제재 관련 비판 |
| 3 | 공지 | 안녕하세요, 소통센터장, 공지 | 운영자 작성 공지 사항 및 안내 |
| 4 | 기타 | (해당 없음) | 일반 게시글, 잡담, 기타 문의 |


---

###  라벨 정의 

| 라벨 | 클래스 | 의미 |
|------|--------|------|
| 0 | 기능건의 | 기능 추가, UX 개선 |
| 1 | 기술오류 | 버그, 결제 오류 |
| 2 | 정책비판 | 운영 및 제재 관련 비판 |
| 3 | 공지 | 운영 공지 |
| 4 | 기타 | 일반 게시글, 잡담 |

---

## 4. 탐색적 데이터 분석 (EDA)

### 4.1 데이터 전처리

모델 학습을 위해 다음과 같은 전처리를 수행하였다.

- 결측치 제거
- post_id 기준 중복 제거
- 숫자형 컬럼 변환 (views, recommends)
- 특수문자 및 HTML 태그 제거
- KoNLPy 기반 불용어 제거
- 명사 토큰화

---

### 4.2 라벨 분포

<img width="800" height="600" alt="Figure_3" src="https://github.com/user-attachments/assets/a9ac58bb-943e-45fa-9474-8ba03552c386" />


---

## 5. 딥러닝 모델 학습 결과

### 5.1 학습 설정

- 모델: `monologg/koelectra-base-v3-discriminator`
- 클래스 수: 5
- Batch Size: 32
- Epoch: 4
- Optimizer: AdamW

---

### 5.2 Epoch별 학습 결과

<img width="1000" height="600" alt="Figure_2" src="https://github.com/user-attachments/assets/c32b189e-3db5-4d99-b768-6c2b23ee5648" />


#### 학습 해석

- Epoch가 증가함에 따라 **Loss는 안정적으로 감소**하였다.
- Validation Accuracy는 약 **94% 수준으로 수렴**하며 높은 성능을 보였다.

---

### 5.3 혼동 행렬 (Confusion Matrix)

<img width="1920" height="973" alt="Figure_1000" src="https://github.com/user-attachments/assets/26a38259-25bb-455f-a35e-c15f922773ff" />

- **결론**: 기술오류(1) 및 정책비판(2) 클래스의 재현율(Recall)이 각각 91.7%, 87.6%로 측정되었다.
이는 모델이 서비스 운영 리스크와 직결되는 이슈를 효과적으로 감지하고 있음을 의미한다.
 
### 5.4 Classification Report

| 클래스 | Precision | Recall | F1-score | Support |
|--------|-----------|--------|----------|---------|
| 기능건의(0) | 0.9258 | 0.9423 | 0.9340 | 1,231 |
| 기술오류(1) | 0.9674 | 0.9170 | 0.9415 | 711 |
| 정책비판(2) | 0.8396 | 0.8761 | 0.8574 | 460 |
| 공지(3) | 0.6914 | 0.6512 | 0.6707 | 86 |
| 기타(4) | 0.9683 | 0.9683 | 0.9683 | 2,493 |
| **전체 정확도** | - | **0.9406** | - | 4,981 |
| **Macro Avg** | 0.8785 | 0.8710 | 0.8744 | 4,981 |
| **Weighted Avg** | 0.9410 | 0.9406 | 0.9406 | 4,981 |


### 모델 성능 핵심 결과

- 기술오류(1) 및 정책비판(2) 클래스의 Recall이 각각 91.7%, 87.6%로,
  서비스 운영 리스크와 직결되는 이슈를 효과적으로 감지할 수 있음을 확인하였다.

- 전체 정확도는 94.06%로 매우 높은 수준이며,
  기타(4)와 기술오류(1) 클래스는 안정적으로 분류되었다.
  반면 공지(3) 클래스는 데이터 부족으로 상대적으로 성능이 낮았다.

---

### 핵심 Recall 성능

| 클래스 | Recall |
|--------|--------|
| 기술오류(1) | **91.7%** |
| 정책비판(2) | **87.6%** |

➡ **운영 리스크 자동 감지에 충분한 모델 성능 확보**

---

## 6. 토픽 모델링 결과 분석 (LDA)

전처리된 전체 게시글 24,905건을 대상으로  
LDA(Latent Dirichlet Allocation) 토픽 모델링을 수행하였으며,  
총 5개의 주요 토픽이 도출되었다.

각 토픽은 워드클라우드 시각화를 통해 핵심 키워드를 분석하였다.

---

### Topic 1: 시청자 / 플랫폼 이용 이슈

<img width="262" height="167" alt="1" src="https://github.com/user-attachments/assets/6bb56fc0-36ef-411f-a60b-ef321b76c6f6" />

**주요 키워드:**  
시청자, 회원, 서비스, 플랫폼, 채널, 유튜브, 방송, 게임

**해석:**  
플랫폼 시청 환경 및 콘텐츠 소비와 관련된 전반적인 사용자 경험을 나타내는 토픽.  
방송 콘텐츠 접근성, 타 플랫폼 비교, 시청 환경 불편 요소 등에 대한 게시글이 포함된다.

---

### Topic 2: 계정 / 제재 / 이용 제한 이슈

<img width="239" height="151" alt="2" src="https://github.com/user-attachments/assets/fc2495e8-3ba0-4ddb-8414-5cc8d9e29230" />


**주요 키워드:**  
계정, 블랙, 정지, 로그인, 인증, 아이디, 차단, 가입

**해석:**  
계정 이용 제한, 제재, 로그인 오류 등  
운영 정책과 계정 관리와 관련된 불만이 집중된 토픽.  
정책비판(라벨 2) 영역과 강하게 연결됨.

---

### Topic 3: 기능 요청 / 서비스 개선 이슈

<img width="252" height="154" alt="3" src="https://github.com/user-attachments/assets/3862c94c-3ef0-40fd-92bf-56d80843af0e" />


**주요 키워드:**  
기능, 채팅, 모바일, 화질, 영상, 관리, 개선

**해석:**  
채팅 시스템 개선, 모바일 시청 환경 보완, 화질 품질 이슈 등  
기능 요구 및 UX 개선 요청이 포함된 토픽.  
기능건의(라벨 0) 클래스와 밀접한 연관이 있음.

---

### Topic 4: 서버 / 운영 / 시스템 이슈

<img width="249" height="156" alt="4" src="https://github.com/user-attachments/assets/917e4818-f295-44e7-bd6a-8fbaa33cc4b4" />


**주요 키워드:**  
서버, 진행, 운영, 중지, 문제, 업데이트

**해석:**  
방송 중단, 서버 장애, 운영 이슈 등  
플랫폼 안정성과 직결되는 핵심 기술 문제를 포함한 토픽.  
기술오류(라벨 1)와 높은 연관성을 가진다.

---

### Topic 5: 고객 문의 / 사용 문의

<img width="251" height="155" alt="5" src="https://github.com/user-attachments/assets/46b80227-7f4d-47d7-908a-780d2f7c7186" />


**주요 키워드:**  
사용, 문의, 제공, 적용, 확인, 댓글, 방송국

**해석:**  
서비스 사용법 문의, 오류 확인 요청 등  
고객센터성 질문이 중심인 토픽.  
기타(라벨 4) 유형 게시글이 주로 포함된다.

---

토픽 모델링 결과,  
SOOP 유저 게시글은 단순 잡담이 아니라 **명확한 유형의 이슈 군집**을 형성하고 있음이 확인되었다.

특히 다음 두 토픽이 가장 중요하다:

- **Topic 2 (계정/정책 이슈)**  
- **Topic 4 (서버/운영 이슈)**  

해당 영역은 서비스 운영 리스크와 직접적으로 연결되며,  
시스템 장애나 정책 변경 이슈 발생 시 게시글 폭증 현상이 나타나는 핵심 구간이다.

이를 통해 자동 분류 모델이 단순 분류를 넘어  

**운영 리스크 감지 시스템으로 활용 가능함** 을 확인하였다.

---

## 7. 결론

본 프로젝트는 SOOP 유저 게시글을 분석하여
서비스 운영에 중요한 이슈를 자동으로 분류하고 감지할 수 있는 모델을 구축하였다.

그 결과, 약 94%의 분류 정확도를 통해
기술오류 및 정책비판과 같은 핵심 운영 리스크를 효과적으로 식별할 수 있었으며,
토픽 모델링 분석을 통해 유저 불만이 특정 이슈 유형을 중심으로 반복 발생함을 확인하였다.

특히 결제 오류와 서버 불안정은 게시글 수보다
서비스 신뢰도에 미치는 영향이 더 크다는 점이 드러났으며,
이에 따라 SOOP 서비스 운영의 최우선 과제로
결제 시스템 안정화와 방송 인프라 개선이 도출되었다.
